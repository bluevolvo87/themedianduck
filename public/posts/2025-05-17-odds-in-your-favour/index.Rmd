...
> For each contestant of the Series 19 of UK Taskmater, generate probabilities their ranking placement in an episode. 
> 
> For example, estimate the probability of Fatiha-El Ghorri placing 1st in an episode of Series 19.
...
```{r, out.width = "75%", fig.cap = "Series Performance so far for Series 19."}
...
ggplot(series_tracker_df, 
       aes(y= Series_Points, x= `Episode ID`, colour = Initials,  group = Initials, image = `Image URL`)) +
    geom_point() +
    geom_line() +
    #geom_image(data = latest_df, aes_(x = ~(`Episode ID` + (Series_Ranking-1)*0.25), y = ~Series_Points , image = ~`Image URL`), by = "height", size = 0.07) +
    #geom_image(data = latest_df, aes(x = 10+(`Series_Ranking`-1)*0.25, y = `Series_Points` , image = `Image URL`), by = "height", size = 0.1) +
...
graph_title_label <- glue(" Distribution Contestant Task Performance for Series: {series_id}", series_id = series_id)
...
- Greater care and understanding of how ties should be handled. 
