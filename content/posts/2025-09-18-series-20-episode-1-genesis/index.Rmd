...
```{r ep1-series-tracking, fig.cap = "The Series 20 Race has begun!", out.width = "75%", cache = FALSE}
title_label <- "Series 20: Series Score Tracker"

latest_df <- series_points_df %>% filter(Episode_ID == 1)
    
series_points_df %>% filter(Episode_ID <= 1) %>%    
ggplot(aes(x= Episode_ID, y= Series_Points)) +
